...
for i_session = 1:size(file_dir_all,1)  
    ...
    wavesurfer_dirname = dir([session_dir, '*.h5']);
    if ~isempty(wavesurfer_dirname) && isscalar(wavesurfer_dirname)
        wavesurfer_dirname = [session_dir wavesurfer_dirname.name];
        single_unit_dir1 = [session_dir, 'Kilosort_SingleUnits1\'];
        single_unit_dir2 = [session_dir, 'Kilosort_SingleUnits2\'];
        obj.load_Wavesurfer_Data(wavesurfer_dirname)
        single_unit_filelist1 = dir([single_unit_dir1, 'SingleUnit*.mat']);
        single_unit_filelist2 = dir([single_unit_dir2, 'SingleUnit*.mat']);
    else
        if isempty(wavesurfer_dirname)
            fprintf('未找到 .h5 文件\n');
        else
            fprintf('找到 %d 个 .h5 文件\n', length(wavesurfer_dirname));
        end
